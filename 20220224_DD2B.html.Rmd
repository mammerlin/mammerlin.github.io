---
title: "DD2B combined with U16S : ZymoBIOMICS and BEI HM-782D Microbial Community Standard"
...
### Data
ZymoBIOMICS D6305 (denoted by Zymo8) and BEI HM-782D (denoted by HM20e) mock communities were used in this demonstration. The sequencing datasets of the mock communities were obtained from the NCBI Sequence Read Archive (SRR9089357, SRR5593316).

The mock communities contain the following bacteria.
...
#### Note that the following species have identical sequences with mock species. Therefore, these species were also considered as correct assignments.
...
U16S is a unified and nonredundant 16S rRNA sequence database created through integration of three widely used databases—RefSeq 16S, SILVA, and EzBioCloud—preceded by systematic taxonomic curation and quality filtering. 
...
setwd("D:/System_bio/Microbiome/Manuscript/")   # CHANGE ME to location of the fastq file
path <- "mock/"
Q30.path <- paste(path,"Q30/",sep="")
input.path <- list.files(Q30.path,pattern=".fastq",include.dirs = T,recursive = T,full.names = T)
nop.path <- file.path(path,"noprimers", basename(input.path))
filter.path <- paste(path,"noprimers/","filtered/",sep="")
...
## Run DDAD2
The following workflow is recommended and demonstrated by *Benjamin J Callahan*.
Please refer to <https://benjjneb.github.io/LRASManuscript/>.
...
bim <- isBimeraDenovo(seqT, minFoldParentOverAbundance=3.5, multithread=T)	
table(bim)
...
##### One chimeras needs to be rmove.


```{r echo=TRUE, message=FALSE,warning=FALSE,eval=FALSE}
#### Remove chimer
seqT.reBim <- removeBimeraDenovo(seqT, method = "per-sample", multithread=T, verbose=TRUE)	

#### 8. assign taxonomy 
DB <- "U16S/U16S.DADA2_format.fa"
tax.reBim <- assignTaxonomy(seqT.reBim, DB, multithread=T,tryRC = F)  ### multithread can be set to TRUE using Linux system
```
...
blast.subject <- "mock/U16S/U16S.blast_20220221.blastdb"
...
#### **Next step is to decide whether the blast results were reliable.**
...
### Check taxonomic assignment by DD2B with mock speices
  
```{r echo=TRUE, message=FALSE,warning=FALSE}
## import the species contain in mock set
mock.species <- read.table("mock/mock.species.txt",header=T,fill=T,quote = "",sep="\t")
mock.species %>% head

```
  
**column mock.sp : documented species contain in mock set.**
**column mock.sp.extend : mock species and those species have identical sequences **

  
### Check HM20e assginment
...
From the comparison table, 20 species of HM20e were assigned correctly.
  
  
### Check Zymo8 assginment
...
From the comparison table, 10 species of Zymo8 were assigned. 
One assigned species, *Bacillus intestinalis*, was misclassified. Bacillus genus is a indistinguishable genus.
